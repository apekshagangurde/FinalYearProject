...
def show_form():
    st.title("Restaurant Data Entry Form")

    # Restaurant form fields
    with st.form("restaurant_form"):
        st.header("Enter Restaurant Details")
        
        restaurant_type = st.selectbox('Select Restaurant Type:', ['Indian', 'Chinese', 'Italian', 'Fast Food'])
        budget = st.slider('Select Budget:', 100, 500, 200)
